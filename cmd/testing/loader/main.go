...
func main() {
	...
	for i := 0; i < Threads; i++ {
		go threadQuery(connURL, wg, i, chanClose, cnanTheadReturn)
	}
	...
	for !exitCondition {
		...
		case n := <-cnanTheadReturn:
			...
			go threadQuery(connURL, wg, n, chanClose, cnanTheadReturn)
		}
	}
...
func threadQuery(connURL string, wg *sync.WaitGroup, threadNumber int,
